...
function updateFileList(files) {
    ...
    // فقط فایل‌های جدید رو اضافه کن (تکراری نشن)
    files.forEach(filename => {
        // چک کن آیا قبلاً وجود داره؟
        const exists = Array.from(filesContainer.children).some(item => 
            item.querySelector('.p-btn')?.textContent === filename
        );

        if (exists) return; // اگر هست، اضافه نکن
        ...
        div.addEventListener('click', async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    editor.setValue(content || '// فایل خالی است');
                    let mode = 'htmlmixed';
                    if (filename.endsWith('.css')) mode = 'css';
                    if (filename.endsWith('.js')) mode = 'javascript';
                    editor.setOption('mode', mode);
                } else {
                    editor.setValue(`// فایل ${filename} یافت نشد (404)`);
                }
            } catch (err) {
...
async function loadFileList() {
    try {
        ...
        console.error('خطا در لود لیست فایل‌ها');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    editor = CodeMirror(document.getElementById("code-editor-container"), {
        value: "// کد اینجا نمایش داده می‌شود\n",
        mode: "htmlmixed",
        theme: "monokai",
        lineNumbers: true,
        lineWrapping: true,
